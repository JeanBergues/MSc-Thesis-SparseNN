...
def return_LassoNet_mask(X1, y1, K=[10], activation='relu', M=10, epochs=500, patience=5, print_lambda=False, print_path=False, plot=False, a=1e-3, nfeat=0):
    ...
    res_k, res_theta, res_val, res_isa = train_lasso_path(dense, starting_lambda, Xt, Xv, yt, yv, ks.optimizers.SGD(learning_rate=a, momentum=0.9), ks.losses.MeanSquaredError(), 
                                                          train_until_k=nfeat, use_faster_fit=True, lr=a, M=M, pm=0.01, max_epochs_per_lambda=1000, use_best_weights=True,
...
def return_LassoNet_estimator(X1, y1, K=[10], activation='relu', M=10, epochs=500, patience=5, print_lambda=False, print_path=False, plot=False, a=1e-3, nfeat=0, drop=0):
    ...
    res_k, res_theta, res_val, res_isa = train_lasso_path(dense, starting_lambda, Xt, Xv, yt, yv, ks.optimizers.SGD(learning_rate=a, momentum=0.9), ks.losses.MeanSquaredError(), 
                                                          train_until_k=nfeat, use_faster_fit=True, lr=a, M=M, pm=0.0005, max_epochs_per_lambda=1000, use_best_weights=True,
...
y_raw = hour_df.close.pct_change(1)[np.max([h_nlags, m_nlags//12]) + 1:].to_numpy().reshape(-1, 1)

Xlist = np.zeros((len(y_raw), 7*(h_nlags + m_nlags)))
for i, hour in enumerate(hour_df.date[np.max([h_nlags, m_nlags // 12]):-1]):
    appending = []
    if h_nlags > 0: appending.append(hour_df[hour_df.date <= hour].tail(h_nlags).iloc[:, 1:-2].to_numpy().ravel())
    if m_nlags > 0: appending.append(min_df[min_df.hdate <= hour].tail(m_nlags).iloc[:, 2:-2].to_numpy().ravel())
    
    Xlist[i] = np.concatenate(appending)
    if i % 100 == 0: print(f"Transformed row {i}")
...
Xtrain, Xtest, ytrain, ytest = ms.train_test_split(Xvoortest, yvoortest, test_size=0.1, shuffle=False)
# print(Xtest[:3][:,3])
# print(yvoortest[:3])
print("Data has been fully transformed and split")

# # LASSO
# predictor = return_lassoCV_estimor(Xtrain, ytrain, cv=5, max_iter=2_000)
...
# # # # My LassoNetw
# mask = return_LassoNet_mask(Xtrain, ytrain, K=[10], activation='tanh', epochs=20_000, patience=100, print_lambda=True, print_path=True, plot=True, nfeat=0)
...
for i in range(n_repeats):
    ...
    predictor = return_MLP_skip_estimator(Xtrain, ytrain, verbose=1, K=[30, 10, 5], activation='tanh', epochs=20_000, patience=50, drop=0, shuff=False)
