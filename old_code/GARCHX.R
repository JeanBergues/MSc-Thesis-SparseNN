...
estimate_garch_model <- function(yt, p, o, q, a, b, X=NULL, summ=FALSE) {
  if (X = NULL) {
    spec <- ugarchspec(
    ...
    spec <- ugarchspec(
      variance.model = list(model = "gjrGARCH", garchOrder = c(p, o, q)),
      mean.model = list(armaOrder = c(a, b), include.mean = TRUE, archm = TRUE, external.regressors = X),
      distribution.model = "std",
    )
...
split_day_data <- split_train_test(day_ret, factor=freq_fact)
...
day_trainX <- as.matrix(day_trainf[c(-1, -5)])[1:(length(day_trainf$close) - 1*freq_fact),]
day_testX <- as.matrix(day_test[c(-1, -5)])[1:(length(day_test$close) - 1*freq_fact),]
day_retX <- as.matrix(day_ret[c(-1, -5)])[1:(length(day_ret$close) - 1*freq_fact),]

ytrain <- day_trainf$close[(1 + 1*freq_fact):length(day_trainf$close)]
ytest <- day_test$close[(1 + 1*freq_fact):length(day_test$close)]
yfull <- day_ret$close[(1 + 1*freq_fact):length(day_ret$close)]
