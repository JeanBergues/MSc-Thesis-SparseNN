...
def return_lassoCV_estimor(X1, y1, cv=5, verbose=0):
    lm = sklm.LassoCV(cv=cv, verbose=verbose, fit_intercept=False)
    lm.fit(X1, y1)
    return lm
...
def return_MLP_skip_estimator(X1, y1, K=[10], activation='relu', epochs=500, patience=30, verbose=0, drop=0):
    Xt, Xv, yt, yv = ms.train_test_split(X1, y1, test_size=0.125, shuffle=True, random_state=rng)
    return train_dense_model(Xt, Xv, yt, yv, 1, ks.optimizers.Adam(1e-3), ks.losses.MeanSquaredError(), ['mse'], activation=activation, neurons=K, verbose=verbose, patience=patience, epochs=epochs, drop=drop)
...
# # My LassoNet
mask = return_LassoNet_mask(Xtrain, ytrain, K=[10], activation='relu', epochs=1000, patience=50, print_lambda=True, print_path=True, plot=True, nfeat=n_selected)
Xtrain = Xtrain[:,mask]
Xtest = Xtest[:,mask]
...
for i in range(n_repeats):
    # predictor = return_lassoCV_estimor(Xtrain, ytrain, cv=5)
    predictor = return_MLP_skip_estimator(Xtrain, ytrain, verbose=0, K=[10], activation='relu', epochs=5000, drop=0)
    ypred = predictor.predict(Xtest).ravel()
...
print(f"Average MSE: {np.mean(results):.4}")
print(f"STD of MSE: {np.std(results):.4}")