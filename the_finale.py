...
def return_lassoCV_estimor(X1, y1, cv=2, verbose=0):
    lm = sklm.LassoCV(cv=cv, verbose=verbose)
    lm.fit(X1, y1)
    return lm
...
def return_MLP_skip_estimator(X1, y1, K=[10], activation='relu', epochs=500, patience=30, verbose=0):
    Xt, Xv, yt, yv = ms.train_test_split(X1, y1, test_size=0.2, shuffle=True, random_state=rng)
...
def return_LassoNet_estimator(X1, y1, K=[10], activation='relu', M=10, epochs=500, patience=5, print_lambda=False, print_path=False, plot=False, a=1e-3, nfeat=0):
    Xt, Xv, yt, yv = ms.train_test_split(X1, y1, test_size=0.2, shuffle=True, random_state=rng)
    ...
    res_k, res_theta, res_val, res_isa = train_lasso_path(dense, starting_lambda, Xt, Xv, yt, yv, ks.optimizers.SGD(learning_rate=a, momentum=0.9), ks.losses.MeanSquaredError(), 
                                                          train_until_k=nfeat, use_faster_fit=True, lr=a, M=M, pm=0.02, max_epochs_per_lambda=100, use_best_weights=True,
...
def return_MLP_estimator(X1, y1, K=[10], activation='relu', patience=30, epochs=500, verbose=0, optimizer=ks.optimizers.Adam(1e-3), loss_func=ks.losses.MeanSquaredError(), metrics=['mse']):
    Xt, Xv, yt, yv = ms.train_test_split(X1, y1, test_size=0.2, shuffle=True, random_state=rng)
...
if True:
    X_pp = pp.StandardScaler().fit(Xlist)
...
# # LASSO
predictor = return_lassoCV_estimor(Xtrain, ytrain, cv=5)
...
# # My LassoNet
predictor, mask = return_LassoNet_estimator(Xtrain, ytrain, K=[5], activation='relu', epochs=1000, patience=30, print_lambda=True, print_path=True, plot=True, nfeat=n_selected)
Xtrain = Xtrain[:,mask]
Xtest = Xtest[:,mask]
ypred = predictor.predict(Xtest).ravel()

# # Paper LassoNet 0.061988
# predictor = return_lassonet_estimor(Xtrain, ytrain, verbose=2, K=(5,))
# ypred = predictor.predict(Xtest).ravel()

# # Regular MLPs
# predictor = return_MLP_skip_estimator(Xtrain, ytrain, verbose=1, K=[5], activation='relu', patience=30, epochs=1000)
# ypred = predictor.predict(Xtest).ravel()
