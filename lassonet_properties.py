...
def results_plot(HP_opts, HP_results, use=(0, 1), title=r"$B$", name="", show=True, log_scale=False):
    ...
    if show:
        ...
        plt.savefig(f'lnprop_plots/LNPROP_{name}.eps', format='eps', bbox_inches='tight')
...
for d_nlags in dlag_opt:
    for h_nlags in use_hlag:
        ...
        HP_results = []
        EXPERIMENT_NAME = "RE_M10_K[100_20]_L[2_48]_X_BVAR_pm02"
        ...
        for hp in HP_opts:
            ...
            if USE_PAPER_LASSONET:
                ...
                res_k, _, res_val, res_l, res_oos = return_LassoNet_results(
                    network, Xt, Xv, yt, yv, pm=0.02, M=10, patiences=(100, 10), max_iters=(1000, hp), print_path=True, print_lambda=True, starting_lambda=None, a=0.01, Xtest=Xtest, ytest=ytest, min_improvement=0.99, steps_back=3)
