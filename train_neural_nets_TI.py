...
def main():
    df = pd.read_csv(f'TI_btc_data.csv')
    ...
    BASE_EXPERIMENT_NAME = "final_forecasts/TI_"
    ...
    for d_nlags in dlag_opt:
        for h_nlags in hlag_opt:
            ...
            X_raw = df.iloc[0:-1].to_numpy()
            y_raw = df.Movement.iloc[1:].to_numpy().reshape(-1, 1)
            ...
            for i in range(n_fm_reps):
                if USE_SKIP:
                    nn = return_MLP_estimator(Xt, Xv, yt, yv, verbose=1, K=best_K, activation=activation, epochs=20_000, patience=fm_patience, drop=dropout, use_L1=use_l1_penalty, es_tol=es_tolerance, lr=learning_rate)
                else:
                    nn = return_MLP_skip_estimator(Xt, Xv, yt, yv, verbose=1, K=best_K, activation=activation, epochs=20_000, patience=fm_patience, drop=dropout, use_L1=use_l1_penalty, es_tol=es_tolerance, lr=learning_rate)
