...
def return_MLP_skip_estimator(X1, y1, K=[10], activation='relu', epochs=500, patience=30, verbose=0, drop=0, shuff=False):
    ...
    return train_dense_model(Xt, Xv, yt, yv, 1, ks.optimizers.Adamax(1e-3), ks.losses.MeanAbsoluteError(), ['mse'], activation=activation, neurons=K, verbose=verbose, patience=patience, epochs=epochs, drop=drop)
...
# # # # My LassoNetw
# mask = return_LassoNet_mask(Xtrain, ytrain, K=[80, 40, 20], activation='relu', epochs=20_000, patience=25, print_lambda=True, print_path=True, plot=True, nfeat=int(Xtrain.shape[1] * 0.15))
...
for i in range(n_repeats):
    ...
    # predictor = return_MLP_skip_estimator(Xtrain, ytrain, verbose=1, K=[40, 20, 10], activation='relu', epochs=20_000, patience=50, drop=0, shuff=True)
    ...
    np.save('forecasts/LassoHour', ypred.ravel())
